...
void _objc_init(void)
{
    ...
    //3.运行系C++静态构造函数(只系统级别)
    //在dyld调用静态析构函数之前，libc会调用_objc_init，即系统级别的C++构造函数先于自定义的C++构造函数运行
    static_init();
    
    //4.runtime运行时环境初始化，分为两部分：
    //unattachedCategories(分类初始化)和allocatedClasses(类的表初始化)
    runtime_init();
    ...
    //8.dyld注册回调函数的地方，以便在映射、取消映射 和初始化objc镜像文件时使用这些回调
    /*
